...
namespace arcball
{
    class util
    {
        ...
        public string node_key = @".\节点坐标及节点特征值.txt";
        public string tri_num = @".\三角单元节点号.txt";
        public string time_info = @".\time.txt";
        public string model_info= @".\model_info.txt";
        ...
        public float d2_Cal_Op_Angle(LINE l0,PO p0)
        {
            ...
            pa = l0.p1;
            pb = l0.p2;
            if (Is_On_Line(pa, pb, p0)) return 0;
            vecab = cal_vec(pa,pb);
            veca0 = cal_vec(pa,p0);
            vecb0 = cal_vec(pb,p0);
            a=vecab.length();
        ...
        public float d2_Cal_Op_Angle(PO pa,PO pb, PO p0)
        {
            //计算夹角的度数
             float a, b, c;
            PO vecab, veca0, vecb0;
        ...
        public void Initial_File()
        {
            FileStream stream = File.Open(infopath, FileMode.OpenOrCreate, FileAccess.Write);
            stream.Seek(0, SeekOrigin.Begin);
            stream.SetLength(0);
            stream.Close();//清空文件内容
            stream = File.Open(node_key, FileMode.OpenOrCreate, FileAccess.Write);
            stream.Seek(0, SeekOrigin.Begin);
            stream.SetLength(0);
            stream.Close();//清空文件内容
            stream = File.Open(tri_num, FileMode.OpenOrCreate, FileAccess.Write);
            stream.Seek(0, SeekOrigin.Begin);
            stream.SetLength(0);
            stream.Close();//清空文件内容
            stream = File.Open(wherepath, FileMode.OpenOrCreate, FileAccess.Write);
            stream.Seek(0, SeekOrigin.Begin);
            stream.SetLength(0);
            stream.Close();//清空文件内容
            stream = File.Open(time_info, FileMode.OpenOrCreate, FileAccess.Write);
            stream.Seek(0, SeekOrigin.Begin);
            stream.SetLength(0);
            stream.Close();//清空文件内容
        ...
        public float CalAgl(PO p1,PO p2)
        {
            //计算以原点为起点的某个向量的终点的与x轴正方向沿逆时针的夹角
            float m1;
            PO p = cal_vec(p1,p2);
        ...
        public PO IsXl(PO s1, PO d1, PO s2, PO d2,ref float f1,ref float f2)
        {
            /*
             * 求出边s1-d1与s2-d2的交点，并返回相应参数
             */
            float D, dl1, dl2;
            PO xpo = new PO();
            D = (s1.x - d1.x) * (d2.y - s2.y) - (d2.x - s2.x) * (s1.y - d1.y);
            ...
            f1 = dl1 / D;
            f2 = dl2 / D;
            xpo.x = f1 * s1.x + (1 - f1) * d1.x;
            xpo.y = f1 * s1.y + (1 - f1) * d1.y;
            return xpo;
        }
        public PO[] Get_po_from_string(string str)
        {
            string sn="";
            float[] fn=new float[8];
            int j=0;
        ...
        //实现边界检测的函数
        public bool Is_On_Line(PO p1, PO p2, PO p0)
        {
            //判断p0是否在p1,p2的连线上，是返回true，不是返回false
            float u;
            u = (p2.y - p1.y) * (p0.x - p1.x) + (p1.x - p2.x) * (p0.y - p1.y);
            if (System.Math.Abs(u) < 0.01)
                return true;
            else return false;
        }
        private float InD0(float a, float b, float c, float x){
		    float d1, d2;
		    d1 = 4 * a*c - b*b;
		    d2 = 2 * a*x + b;
		    if (d1 < 0)
                return (float)(System.Math.Log10(System.Math.Abs((d2 - System.Math.Pow(-d1, 0.5)) / (d2 + System.Math.Pow(-d1, 0.5)))) / System.Math.Log10(System.Math.Exp(1)));
		    else
			    return (float)(2 * System.Math.Atan(d2 /System.Math.Pow(d1, 0.5)) /System.Math.Pow(d1, 0.5));
	    }
        private float InD1(PO p1, PO p2, PO p0)
        {
            float k, a, b, c, di, bi, m;
            if (Is_On_Line(p1, p2, p0)) return 0;
            if (p1.y == p2.y)
            {
                return (float)((System.Math.Atan(p2.x - p0.x) / (p0.y - p1.y)) - System.Math.Atan((p1.x - p0.x) / (p0.y - p1.y)));
            }
            else if (p1.x == p2.x)
            {
                return (float)(System.Math.Atan((p2.y - p0.y) / (p1.x - p0.x)) - System.Math.Atan((p1.y - p0.y) / (p1.x - p0.x)));
            }
            else
            {
                ...
                bi = p1.y - k * p1.x;
                a = 1 + k * k;
                b = -2 * p0.x + 2 * k * (bi - p0.y);
                c = (p0.y - bi) * (p0.y - bi) + p0.x * p0.x;
                m = b * b - 4 * a * c;
                di = p0.y - k * p0.x - bi;
                if (m == 0) return di * (1 / (p1.x - m) - 1 / (p2.x - m));
                return di * (InD0(a, b, c, p2.x) - InD0(a, b, c, p1.x));
            }
        }
        public bool Is_in_polygon(LinkList<PO> pl,PO p0)
        {
            ...
            while (pnt.Next!=null)
            {
                sum += InD1(pnt.Data,pnt.Next.Data,p0);
                pnt= pnt.Next;
            }
            sum += InD1(pnt.Data,pl.Head.Data, p0);
            //u1.InFile(u1.infopath,sum);
            if (System.Math.Abs(sum)<5)
                return false;
            else
                return true;
        }
        public int Cal_po_on_Line(PO p1, PO p2, float y)
        {
            /*
             * 根据p1-p2所确定的线，和一个点的y坐标确定其x坐标,并取整
             */
            return (int)((y - p1.y) * (p1.x - p2.x) / (p1.y - p2.y) + p1.x);
        }
        public void Tip_Time()
        {
            now_time = DateTime.Now;
        }       
        public void Span_from_last_time(String str)
        {
            DateTime dn=DateTime.Now;
            TimeSpan span = dn.Subtract(now_time);
            InFile(time_info,str+"----"+span.Milliseconds);
        }
